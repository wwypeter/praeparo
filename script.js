...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // =========================================================
  const scrollOffsets = {
    'kontakt': 500,              // Einzelberatung DESKTOP
    'sprechstundentermine': 50, // Sprechstunde DESKTOP
    'preise': 50,               // Preise DESKTOP
    'faq': 70                   // FAQ DESKTOP
  };

 const scrollOffsetsMobile = {
    'kontakt': 400,               // Einzelberatung MOBILE - HIER ANPASSEN
...
// =========================================================
// MOBILE: AUSKLAPPBARE TEXTE IN HAUPTSEKTIONEN
// =========================================================
(function() {
  ...
  function initMobileTextToggle() {
    ...
    console.log('Prüfe Bildschirmbreite:', window.innerWidth, '- Mobile:', isMobile);
    
    // Alle 5 Hauptsektionen
    const sections = ['#s1', '#s2', '#s3', '#m', '#p'];
    ...
    sections.forEach(sectionId => {
      ...
      // Entferne alte Toggle-Elemente falls vorhanden
      const existingWrapper = contentDiv.querySelector('.content-inner');
      const existingButton = contentDiv.querySelector('.content-toggle-btn');
      
      if (isMobile && !togglesInitialized) {
        // MOBILE: Initialisiere Toggle
        console.log('Initialisiere Toggle für:', sectionId);
        
        const h2 = contentDiv.querySelector('h2');
        const beratungActions = contentDiv.querySelector('.beratung-actions');
        
        // Erstelle Wrapper nur wenn er noch nicht existiert
        if (!existingWrapper) {
          const wrapper = document.createElement('div');
          wrapper.className = 'content-inner';
          
          // Finde alle Elemente zwischen h2 und beratung-actions
          let currentElement = h2 ? h2.nextElementSibling : contentDiv.firstElementChild;
          const elementsToWrap = [];
          
          while (currentElement && currentElement !== beratungActions) {
            elementsToWrap.push(currentElement);
            currentElement = currentElement.nextElementSibling;
          }
          
          // Verschiebe Elemente in Wrapper
        ...
        // Erstelle Button nur wenn er noch nicht existiert
        if (!existingButton) {
          const toggleBtn = document.createElement('button');
          toggleBtn.className = 'content-toggle-btn';
          toggleBtn.setAttribute('aria-label', 'Text ein-/ausklappen');
          toggleBtn.innerHTML = `
          ...
          const beratungActions = contentDiv.querySelector('.beratung-actions');
          if (beratungActions) {
            contentDiv.insertBefore(toggleBtn, beratungActions);
          } else {
        ...
        contentDiv.classList.add('has-toggle');
        
      } else if (!isMobile && togglesInitialized) {
        // DESKTOP: Entferne Toggle-Elemente
        console.log('Entferne Toggle für:', sectionId);
        
        if (existingWrapper) {
          // Verschiebe Inhalte aus Wrapper zurück
          const h2 = contentDiv.querySelector('h2');
        ...
        contentDiv.classList.remove('has-toggle', 'expanded');
      }
    });
    
    togglesInitialized = isMobile;
  }
  
  // Initiale Ausführung
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initMobileTextToggle);
  } else {
    initMobileTextToggle();
  }
  
  // Bei Fenstergrößen-Änderung erneut prüfen
  let resizeTimer;
  window.addEventListener('resize', function() {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(initMobileTextToggle, 250);
  });
})();